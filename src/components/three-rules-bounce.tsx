...
export function ThreeRulesBounce() {
  ...
  useEffect(() => {
    ...
    const updateSize = () => {
      const containerWidth =
        pre.parentElement?.offsetWidth || window.innerWidth;
      const containerHeight =
        pre.parentElement?.offsetHeight || window.innerHeight;
      return {
    ...
    const positions = [
      ...
      { x: Math.floor(width * 0.5), y: Math.floor(height / 2), dx: -1, dy: 1 },
  ...
  return (
    <div className="w-full h-full overflow-hidden">
      <pre ref={preRef} className="text-base-content bg-base-300" />
    </div>
  );
}
